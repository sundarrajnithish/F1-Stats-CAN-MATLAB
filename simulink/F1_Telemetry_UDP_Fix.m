% F1_Telemetry_UDP_Fix.m
% Creates a Simulink model for F1 telemetry that receives data via UDP
% Works without Vector CAN hardware
% Fixed version with correct display parameters

% Define a unique model name to avoid shadowing
modelName = 'F1_Telemetry_UDP_Fix';
...
% Add display blocks - with FIXED PARAMETERS that work in all MATLAB versions
...
fprintf('   python UDP_Test.py   (for test data)\n');
fprintf('   python UDP_Send.py   (for real F1 data)\n\n');
fprintf('To stop the system and clean up resources:\n');
fprintf('   stopF1UDP_Fix\n\n');

% Create stop function
stopFcn = sprintf(['function stopF1UDP_Fix()\n', ...
    '    try\n', ...
...
function receiveUDPData(~, ~)
    try
        ...
        if ~isempty(udpReceiver)
            ...
            if ~isempty(data)
                if length(data) >= 7 % Expect at least 7 bytes
